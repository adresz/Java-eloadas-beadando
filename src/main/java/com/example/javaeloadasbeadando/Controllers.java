...
public class Controllers {
    ...
    public class AccountController {
        ...
        public String getAccountSummary(Model model) {
            AccountSummary s;
            try {
                s = Config.getContext().account.summary(Config.ACCOUNTID).getAccount();
            } catch (Exception e) {
                model.addAttribute("error", "❌ Hiba történt az adatok lekérésekor!");
                return "account"; // ugyanaz a template, hibát itt tudod kezelni
            }

            model.addAttribute("summary", s);
            return "account"; // account.html template
        }
    }
